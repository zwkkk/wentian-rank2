...
class Pooler(nn.Module):
    ...
    def __init__(self, pooler_type):
        ...
        assert self.pooler_type in ["cls", "cls_before_pooler", "avg", "avg_top2","avg_top4",
    ...
    def forward(self, attention_mask, outputs):
        ...
        if self.pooler_type in ['cls_before_pooler', 'cls']:
            ...
            pooled_result = ((last_hidden + second_last_hidden) / 2.0 * attention_mask.unsqueeze(-1)).sum(
                1) / attention_mask.sum(-1).unsqueeze(-1)
            return pooled_result
        elif self.pooler_type == "avg_top4":
            fourth_last_hidden = hidden_states[-4]
            third_last_hidden = hidden_states[-3]
            second_last_hidden = hidden_states[-2]
            last_hidden = hidden_states[-1]
            pooled_result = ((last_hidden + second_last_hidden + third_last_hidden + fourth_last_hidden) / 4.0 * attention_mask.unsqueeze(-1)).sum(
                1) / attention_mask.sum(-1).unsqueeze(-1)
            return pooled_result
        else:
...
def cl_forward(cls,
    ...
    #dropouts = MultiSampleDropout(config, pooler_num=pooler_num, dropout_num=2)
    #pooler_output = dropouts(pooler_output)
    # If using "cls", we add an extra MLP layer
    # (same as BERT's original implementation) over the representation.
    if cls.pooler_type == "cls":
        out = cls.mlp(pooler_output)
        
    # Separate representation
    z1, z2 = out[:, 0], out[:, 1] # z1 : query, z2: doc
    ...
    cos_sim = cls.sim(z1.unsqueeze(1), z2.unsqueeze(0)) # 计算query和doc的相似度 in-batch nagetive
    # Hard negative
    if num_sent >= 3:
    ...
    loss = loss_fct(cos_sim, labels) # loss
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, pooler_type='cls', temp=0.05, pooler_num=128, do_mlm=False, mlm_weight=None):
        ...
        if 'nezha' not in self.model_name_or_path:
