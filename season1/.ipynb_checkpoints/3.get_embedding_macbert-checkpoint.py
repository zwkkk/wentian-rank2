...
device = "cuda:1"
tokenizer = AutoTokenizer.from_pretrained("./result/roberta_result")
batch_size = 100
use_pinyin = False
...
if __name__ == '__main__':
    model = BertForCL.from_pretrained("./result/roberta_result")
    model.to(device)
    ...
    if not os.path.exists('embedding'):
        ...
        os.makedirs('embedding/roberta_result')
    
    query_embedding_file = csv.writer(open('embedding/roberta_result/query_embedding', 'w'), delimiter='\t')
    ...
    doc_embedding_file = csv.writer(open('embedding/roberta_result/doc_embedding', 'w'), delimiter='\t')
