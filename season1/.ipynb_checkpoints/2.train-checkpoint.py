...
from config.configuration_nezha import NeZhaConfig
#nazhe
MODEL_CLASSES = {
    'nezha': (NeZhaConfig, BertForCL, BertTokenizer)
}
...
def main():
    ...
    if extension == "csv":
        datasets = load_dataset(extension, data_files=data_files, cache_dir="./data/",
                                delimiter="\t" if "tsv" in data_args.train_file else ",") # load dataset
    else:
        datasets = load_dataset(extension, data_files=data_files, cache_dir="./data/")
    ...
    if model_args.config_name:
        ...
        if 'nezha' in model_args.model_name_or_path:
            config = NeZhaConfig.from_pretrained(model_args.model_name_or_path, **config_kwargs)
        else:
    ...
    def prepare_features(examples):
        ...
        sentences = examples[sent0_cname] + examples[sent1_cname] #将query和doc拼接成一句
        ...
        sent_features = tokenizer( # 新的句子tokenizer
            sentences,
        ...
        return features # {每一个字符:对应的tokenizer结果}

    '''
    准备训练集，测试集 用于模型输入的datasets
    '''
