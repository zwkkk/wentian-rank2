...
import json
device = "cuda:0"
tokenizer = AutoTokenizer.from_pretrained("./result/macbert_result/")
batch_size = 100
use_pinyin = False

if os.path.exists('../season2/data/train.query.json'):
    os.remove('../season2/data/train.query.json')
if os.path.exists('../season2/data/corpus.json'):
    os.remove('../season2/data/corpus.json')
...
if __name__ == '__main__':
    ...
    '''
    model = BertForCL.from_pretrained("./result/macbert_result")
    model.to(device)
    corpus = [line[1] for line in csv.reader(open("./data/season2_process/corpus.tsv"), delimiter='\t')]
    query = [line[1] for line in csv.reader(open("./data/season2_process/train.query.txt"), delimiter='\t')]
    ...
    for i in tqdm(range(0, len(query), batch_size)):
        ...
        for j in range(len(temp_embedding)):
            ...
            write_to_json(query_dict, '../season2/data/train.query.json')
    ...
    for i in tqdm(range(0, len(corpus), batch_size)):
        ...
        for j in range(len(temp_embedding)):
            ...
            write_to_json(doc_dict, '../season2/data/corpus.json')
