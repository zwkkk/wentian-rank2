...
class RankModel(object):
    ...
    def create_model(self):
        ...
        with tf.variable_scope("loss"):
            ...
            self.probabilities_float = tf.to_float(self.probabilities)

        if self.is_training:
            with tf.name_scope("train_op"):
                self.train_op = optimization.create_optimizer(
                    self.loss, self.learning_rate, self.num_train_steps, self.num_warmup_steps, use_tpu=False)
        self.score = tf.identity(self.probabilities_float, name="score")
...
if __name__ == "__main__":
    # trainer 保存到result，wrapper保存到model
    ...
    parser.add_argument("--ckpt_to_convert", type=str, default='./result/bert_base_neg6_hard/models-64000', help="the checkpoint of trained model to convert")
    parser.add_argument("--output_dir", type=str, default='./model/bert_base_neg6_hard', help="the path of saved_model")
